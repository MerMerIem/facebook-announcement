...
const AdminDashboard = () => {
  ...
  const handleStatusChange = (orderId, newStatus) => {
    ...
    toast({
      ...
      description: `Le statut de la commande a été changé à ${getStatusLabel(newStatus)}.`,
    });
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="bg-gradient-card shadow-card border-0">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Totale Commandes</CardTitle>
          ...
          <Card className="bg-gradient-card shadow-card border-0">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Totale Revenu</CardTitle>
              <Package className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {orders.reduce((sum, order) => sum + order.total, 0).toFixed(2)} DA
              </div>
            </CardContent>
          </Card>
          <Card className="bg-gradient-card shadow-card border-0">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Moyenne Commande</CardTitle>
              <Package className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {orders.length > 0 ? (orders.reduce((sum, order) => sum + order.total, 0) / orders.length).toFixed(2) : '0.00'} DA
              </div>
            </CardContent>
          </Card>
        </div>
                      ...
                      <TableRow key={order.id}>
                        ...
                        <TableCell>
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">{order.productPrice} DA</div>
                          </div>
                        </TableCell>
                        <TableCell>
                          ...
                          <Select value={order.status} onValueChange={(value) => handleStatusChange(order.id, value)}>
                            ...
                            <SelectContent>
                              <SelectItem value="pending">
                                <Badge variant={getStatusBadgeVariant('pending')}>En attente</Badge>
                              </SelectItem>
                              <SelectItem value="delivered">
                                <Badge variant={getStatusBadgeVariant('delivered')}>Livrée</Badge>
                              </SelectItem>
                              <SelectItem value="canceled">
                                <Badge variant={getStatusBadgeVariant('canceled')}>Annulée</Badge>
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        </TableCell>
