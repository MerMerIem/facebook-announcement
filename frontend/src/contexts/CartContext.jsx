...
const calculateTotal = (items, pricingData = null) => {
    ...
    return items.reduce((total, item) => {
        const hasDiscount =
            item.product?.used_discount === true ||
            item.product?.used_discount === 1;
                ...
                const newItem = {
                    ...
                    _itemType: {
                        ...
                        hasVariant: !!(
                            ...
                            action.product.selectedVariant
                        ),
                        hasCustomOptions: !!(
                            action.product.custom_options &&
                            Object.keys(action.product.custom_options).length >
                                0
                        ),
                        hasBatch: !!(
                            action.product.batch_number ||
                            action.product.expiry_date
                        ),
                        hasPriceTier: !!(
                            action.product.price_tier ||
                            action.product.applied_discount
                        ),
                    },
                };
        ...
        case 'UPDATE_QUANTITY':
            if (action.quantity <= 0) {
                ...
                newItems = state.items.map(item => {
                    ...
                    if (
                        action.cartItemId &&
                        item._cartItemId === action.cartItemId
                    ) {
        ...
        case 'LOAD_CART':
            const loadedItems = (action.items || []).map(item => ({
                ...
                _itemType: item._itemType || {
                    ...
                    hasBatch: !!(
                        item.product?.batch_number || item.product?.expiry_date
                    ),
                    hasPriceTier: !!(
                        item.product?.price_tier ||
                        item.product?.applied_discount
                    ),
                },
            }));
        ...
        case 'MERGE_SIMILAR_ITEMS':
            ...
            state.items.forEach(item => {
                ...
                if (shouldCreateSeparateLineItem(item.product)) {
                    ...
                    const totalQuantity = similarItems.reduce(
                        (sum, similarItem) => {
...
export const CartProvider = ({ children }) => {
    ...
    const fetchPricing = async () => {
        ...
        try {
            ...
            const [pricingResult, response, responseCode, error] =
                await api.post('/order/calculate-pricing', requestBody, {
