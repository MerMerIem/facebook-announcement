...
const Cart = () => {
        ...
        if (pricingData?.pricing_details) {
            ...
            if (anyProductPricing) {
                return {
                    ...
                    savings:
                        (anyProductPricing.original_price -
                            anyProductPricing.unit_price) *
                        item.quantity,
                    _isEstimated: true,
                };
            }
    ...
    const getProductName = item => {
        ...
        return (
            item.product.name || item.product.product_name || 'منتج غير محدد'
        );
    };
    ...
    const prepareCheckoutData = () => {
        const checkoutItems = items.map(item => {
            ...
            return {
                ...
                subcategory:
                    item.product.subcategory?.name || item.product.subcategory,
    ...
    return (
        <div dir="rtl" className="min-h-screen bg-shop-bg">
            <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8">
                ...
                <div className="flex flex-col lg:grid lg:grid-cols-3 gap-5 sm:gap-7 lg:gap-10">
                    {/* Cart Items Section */}
                    <div
                        className="order-2 lg:order-1 lg:col-span-2 space-y-4 sm:space-y-5"
                        dir="rtl"
                    >
                        ...
                        <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-3 sm:gap-4">
                            ...
                            <Button
                                ...
                                size="sm"
                                onClick={clearCart}
                                className="text-destructive bg-white! hover:bg-red-400! hover:text-white! w-full xs:w-auto text-sm rounded-none"
                            >
                        ...
                        <div className="space-y-2 sm:space-y-3">
                            {items.map(item => {
                                ...
                                const hasMeasureUnit =
                                    item.product.has_measure_unit === 1 ||
                                    item.product.has_measure_unit === true;
                                const measureUnit =
                                ...
                                const serverItemDetails =
                                    getServerItemDetails(item);

                                const hasDiscount =
                                    serverItemDetails?.used_discount ||
                                    (item.product.has_discount &&
                                        new Date(item.product.has_discount) >
                                            new Date());
                                ...
                                const itemTotal =
                                    serverItemDetails?.item_total ||
                                    itemPrice * item.quantity;

                                // Check if we're using estimated pricing
                                const isEstimated =
                                    serverItemDetails?._isEstimated;

                                const isEditingThisQuantity =
                                ...
                                return (
                                    <Card
                                        ...
                                        className={`border border-gray-200 rounded-none bg-white shadow-none transition-all duration-200 ${
                                    ...
                                    >
                                        <CardContent className="p-3 sm:p-4 lg:p-6">
                                            <div className="flex gap-3 sm:gap-4 lg:gap-5">
                                                ...
                                                <div className="w-16 h-16 xs:w-20 xs:h-20 sm:w-24 sm:h-24 lg:w-32 lg:h-32 bg-gray-50 rounded-none overflow-hidden flex-shrink-0 relative">
                                                    <img
                                                        src={
                                                            item.product
                                                                .main_image_url ||
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex justify-between items-start mb-2 gap-2">
                                                        ...
                                                        <Button
                                                            ...
                                                            className="text-gray-400 hover:text-red-500 hover:bg-red-50 p-1.5 h-auto rounded-none flex-shrink-0"
                                                        >
                                                            <Trash2 className="h-3 w-3 sm:h-4 sm:w-4" />
                                                        </Button>
                                                    </div>
                                                    ...
                                                    {hasVariant &&
                                                        item.variant && (
                                                            <div className="text-xs sm:text-sm text-gray-500 mb-2 bg-gray-50 px-2 py-1 rounded-none text-right">
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-1 sm:gap-2 mb-2">
                                                        {serverItemDetails?.special_pricing && (
                                                            <div className="text-xs text-green-600 bg-green-50 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-none">
                                                                تسعير خاص متاح
                                                            </div>
                                                        )}
                                                        {isLoadingPrice && (
                                                            <div className="text-xs text-blue-600 bg-blue-50 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-none flex items-center gap-1">
                                                        ...
                                                        {isEstimated &&
                                                            !isLoadingPrice && (
                                                                <div className="text-xs text-yellow-600 bg-yellow-50 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-none">
                                                                    سعر تقديري
                                                                </div>
                                                            )}
                                                    </div>
                                                    ...
                                                    <div className="space-y-3">
                                                        ...
                                                        <div className="text-right">
                                                            ...
                                                            <div className="flex flex-wrap items-center gap-1 sm:gap-2">
                                                                ...
                                                                {savings >
                                                                    0 && (
                                                                    <span className="text-xs bg-green-100 text-green-700 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-none font-medium">
                                                                        توفير{' '}
                                                                        {formatPrice(
                                                                            savings
                                                                        )}
                                                        ...
                                                        <div className="flex justify-end">
                                                            <div
                                                                className={`flex items-center bg-gray-100 rounded-none border transition-all duration-200 ${
                                                                    isLoadingPrice
                                                                        ? 'opacity-75'
                                                                        : ''
                                                                }`}
                                                            >
                                                                <Button
                                                                    variant="ghost"
                                                                    size="sm"
                                                                    className="h-8 w-8 sm:h-10 sm:w-10 p-0 hover:bg-gray-200 rounded-none"
                                                                    onClick={() =>
                    ...
                    <div className="order-1 lg:order-2 lg:col-span-1" dir="rtl">
                        <Card className="lg:sticky lg:top-20 border-border bg-white lg:shadow-none rounded-none">
                                ...
                                <div className="space-y-2" dir="rtl">
                                    ...
                                    {loadingPricing ? (
                                        <div className="text-center py-3">
                                            <div className="animate-spin rounded-none h-4 w-4 border-b-2 border-primary mx-auto"></div>
                                                        ...
                                                        wilaya => (
                                                            ...
                                                            >
                                                                <div className="flex justify-between items-center w-full p-1">
                                                                    ...
                                                                    <span
                                                                        className={`text-sm font-semibold px-2 py-0.5 rounded-none ml-2 
        ${wilaya.delivery_fee === 0 ? ' text-green-700' : ' text-primary'}`}
                                ...
                                {!selectedWilayaId && (
                                    <div className="bg-yellow-50 border border-yellow-200 rounded-none p-3">
                                        <p className="text-sm text-yellow-800 text-right leading-relaxed">
                                            يرجى اختيار الولاية لحساب رسوم
                                            التوصيل وإتمام الطلب
                                        </p>
                                    </div>
                                )}
                                ...
                                <div className="space-y-2">
                                    ...
                                    {selectedWilayaInfo && (
                                        <div className="text-xs sm:text-sm text-muted-foreground bg-gray-50 px-2 py-1 rounded-none">
                                            التوصيل إلى:{' '}
                                            {selectedWilayaInfo.wilaya_name}
                                        </div>
                                    )}
                                ...
                                <div className="space-y-2 sm:space-y-3 pt-2">
                                    {selectedWilayaId ? (
                                        <Button
                                            variant="default"
                                            className="w-full rounded-none bg-primary hover:bg-amber-700 transition-all duration-200 text-sm sm:text-base py-2.5 sm:py-3"
                                            size="lg"
                                        ...
                                        <Button
                                            className={`w-full rounded-none transition-all duration-200 text-sm sm:text-base py-2.5 sm:py-3 ${
                                                !selectedWilayaId
                                    ...
                                    <Link to="/shop" className="block">
                                        <Button
                                            variant="ghost"
                                            className="w-full rounded-none border-primary text-primary hover:bg-primary! hover:text-white text-sm sm:text-base py-2.5 sm:py-3"
                                        >
                                            متابعة التسوق
                                        </Button>
                                    </Link>
