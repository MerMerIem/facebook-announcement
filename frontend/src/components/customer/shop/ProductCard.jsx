...
const ProductCard = ({ product }) => {
        ...
        const fetchSpecialPricing = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (
                        result.success &&
                        result.data.pricing_details.length === 2
                    ) {
    ...
    return (
        <Card
            dir="rtl"
            className="group hover-lift animate-fade-in relative overflow-hidden h-full rounded-none border-border shadow-none bg-white p-0 flex flex-col"
            ...
            <div className="flex flex-col flex-grow">
                                ...
                                {specialPricing && !loadingPricing && (
                                    <div className="bg-green-50 border border-green-200 rounded-lg p-2 space-y-1">
                                        ...
                                        <div className="text-xs text-green-600 font-medium">
                                            وفر{' '}
                                            {formatPrice(
                                                specialPricing.savings
                                            )}{' '}
                                            ({specialPricing.savingsPercentage}
                                            %) لكل قطعة إضافية
                                        </div>
                                    </div>
                                )}
                ...
                <div className="p-3 sm:p-4 pt-0 space-y-2 mt-auto">
                    <Button
                        ...
                        className="w-full text-xs sm:text-sm rounded-none"
                        size="sm"
                    >
                    ...
                    {itemQuantity > 0 && specialPricing && (
                        <Button
                            onClick={handleAddToCart}
                            variant="outline"
                            className="w-full rounded-none border-green-500 text-green-600 hover:bg-green-50 text-xs sm:text-sm"
                            size="sm"
                        >
