           ...
           AND variant_id = ?`,
            ...
            if (currentVariantImages && currentVariantImages.length > 0) {
                ...
                for (const [
                    imgIndex,
                    image,
                ] of currentVariantImages.entries()) {
                    await conn.execute(
                        `INSERT INTO product_variant_images (variant_id, url, public_id, is_primary, sort_order) VALUES (?, ?, ?, ?, ?)`,
                        [
                            variant.id,
    ...
    try {
        ...
        if (ordersWithVariant[0].count > 0) {
            ...
            res.status(200).json({
                success: true,
                message:
                    'تم إلغاء تفعيل المتغير بنجاح (المتغير مستخدم في طلبات سابقة)',
                data: {
...
export async function getVariants(req, res) {
    ...
    `,
        ...
        const formattedVariants = variants.map(variant => {
            ...
            return {
                ...
                orders: {
                    ...
                    totalQuantityOrdered: parseInt(
                        variant.total_quantity_ordered
                    ),
                ...
                status: {
                    ...
                    hasActiveDiscount: discountInfo
        ...
        const priceRange = {
            min: Math.min(
                ...formattedVariants.map(v => v.pricing.currentPrice)
            ),
            max: Math.max(
                ...formattedVariants.map(v => v.pricing.currentPrice)
            ),
        ...
        console.log(
            `Found ${variants.length} variants for product ${productId}`
        );
