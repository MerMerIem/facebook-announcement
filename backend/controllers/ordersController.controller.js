  ...
  try {
    ...
    for (const item of processedItems.values()) {
      ...
      if (
        ...
        unitPrice =
          basePrice * (1 - Number(product.discount_percentage || 0) / 100);
        specialPricing = true;
      }
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (
        ...
        unitPrice = basePrice * (1 - Number(product.discount_percentage) / 100);
        specialPricing = true;
      }
  ...
  try {
    ...
    const [itemsResult] = await db.execute(
      `SELECT 
        ...
        p.has_measure_unit AS product_has_measure_unit,
        p.measure_unit AS product_measure_unit,
        p.allows_custom_quantity AS product_allows_custom_quantity,
        ...
        pv.allows_custom_quantity AS variant_allows_custom_quantity,
    ...
    const items = itemsResult.map((item) => {
      const usesCustomQuantity = item.variant_id
        ? !!item.variant_allows_custom_quantity
        : !!item.product_allows_custom_quantity;

      const measureUnit = item.variant_id
        ? item.variant_measure_unit
        : item.product_measure_unit;
      ...
      return {
        ...
        display_name: item.variant_title
          ...
          : item.product_name,
        uses_custom_quantity: usesCustomQuantity,
        measure_unit: measureUnit, // e.g. "متر", "كغ", "لتر"
      };
    });
